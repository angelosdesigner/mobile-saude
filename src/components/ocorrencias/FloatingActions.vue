...
<template>
  ...
  <div v-else class="flex items-center gap-1 rounded-2xl bg-white px-2 py-2 shadow-lg">
    ...
    <template v-for="a in actions" :key="a.key">
      <span class="mx-0.5 h-6 w-px bg-ms-border-light" />
      <el-tooltip :content="a.tip" placement="top">
        <button
          class="flex h-9 w-9 items-center justify-center rounded-xl transition hover:bg-ms-fill-light"
          :class="a.green ? 'text-ms-success' : 'text-ms-text-regular hover:text-ms-primary'"
        ...
        >
          ...
          >
            ...
            <template v-else-if="a.icon === 'whatsapp'">
              <path
                d="M21 11.5a8.38 8.38 0 0 1-8.5 8.5 8.5 8.5 0 0 1-3.8-.9L3 21l1.9-5.7A8.5 8.5 0 1 1 21 11.5z"
              />
            </template>
            <template v-else-if="a.icon === 'video'">
              <path d="m23 7-7 5 7 5z" />
              <rect x="1" y="5" width="15" height="14" rx="2" />
            </template>
            <template v-else-if="a.icon === 'paperclip'">
              <path
                d="M21.4 11.05 12.25 20.2a5 5 0 0 1-7.07-7.07l9.19-9.19a3 3 0 1 1 4.24 4.24l-9.2 9.19a1 1 0 0 1-1.41-1.41l8.49-8.49"
              />
            </template>
            <template v-else-if="a.icon === 'transfer'">
              <path
    ...
    <el-dropdown trigger="click" placement="top" @command="(c: string) => emit('action', c)">
      <button
        class="flex h-9 w-9 items-center justify-center rounded-xl text-ms-text-regular transition hover:bg-ms-fill-light hover:text-ms-primary"
        title="Mais ações"
      >
